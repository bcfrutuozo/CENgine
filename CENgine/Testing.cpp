...
void TestDynamicConstant()
{
	...
	{
		DRR::IncompleteLayout s;
		s.Add<DRR::Struct>( "butts"s );
		s["butts"s].Add<DRR::Float3>( "pubes"s );
		s["butts"s].Add<DRR::Float>( "dank"s );
		s.Add<DRR::Float>( "woot"s );
		s.Add<DRR::Array>( "arr"s );
		s["arr"s].Set<DRR::Struct>( 4 );
		s["arr"s].T().Add<DRR::Float3>( "twerk"s );
		s["arr"s].T().Add<DRR::Array>( "werk"s );
		s["arr"s].T()["werk"s].Set<DRR::Float>( 6 );
		s["arr"s].T().Add<DRR::Array>( "meta"s );
		s["arr"s].T()["meta"s].Set<DRR::Array>( 6 );
		s["arr"s].T()["meta"s].T().Set<DRR::Matrix>( 4 );
		s["arr"s].T().Add<DRR::Bool>( "booler" );

		// fails: duplicate symbol name
		// s.Add<DRR::Bool>( "arr"s );

		// fails: bad symbol name
		//s.Add<DRR::Bool>( "69man" );

		auto b = DRR::Buffer( std::move( s ) );
	...
	// size test array of arrays
	{
		DRR::IncompleteLayout s;
		s.Add<DRR::Array>( "arr" );
		s["arr"].Set<DRR::Array>( 6 );
		s["arr"].T().Set<DRR::Matrix>( 4 );
		auto b = DRR::Buffer( std::move( s ) );
	...
	// size test array of structs with padding
	{
		DRR::IncompleteLayout s;
		s.Add<DRR::Array>( "arr" );
		s["arr"].Set<DRR::Struct>( 6 );
		s["arr"s].T().Add<DRR::Float2>( "a" );
		s["arr"].T().Add<DRR::Float3>( "b"s );
		auto b = DRR::Buffer( std::move( s ) );
	...
	// testing CookedLayout
	{
		...
		s.Add<DRR::Array>( "arr" );
		s["arr"].Set<DRR::Float3>( 6 );
		auto cooked = DRR::LayoutCodex::Resolve( std::move( s ) );
		// raw is cleared after donating
		s.Add<DRR::Float>( "arr" );
		// fails to compile, cooked returns const&
		// cooked["arr"].Add<DRR::Float>("buttman");
		auto b1 = DRR::Buffer( cooked );
		b1["arr"][0] = dx::XMFLOAT3{ 69.0f,0.0f,0.0f };
		auto b2 = DRR::Buffer( cooked );
	...
	// specific testing scenario (packing error)
	{
		...
		auto cooked = DRR::LayoutCodex::Resolve( std::move( pscLayout ) );
		assert( cooked.GetSizeInBytes() == 48u );
	}
	// array non-packing
	{
		DRR::IncompleteLayout pscLayout;
		pscLayout.Add<DRR::Array>( "arr" );
		pscLayout["arr"].Set<DRR::Float>( 10 );
		auto cooked = DRR::LayoutCodex::Resolve( std::move( pscLayout ) );
	...
	// array of struct w/ padding
	{
		DRR::IncompleteLayout pscLayout;
		pscLayout.Add<DRR::Array>( "arr" );
		pscLayout["arr"].Set<DRR::Struct>( 10 );
		pscLayout["arr"].T().Add<DRR::Float3>( "x" );
		pscLayout["arr"].T().Add<DRR::Float2>( "y" );
		auto cooked = DRR::LayoutCodex::Resolve( std::move( pscLayout ) );
	...
	// specific testing scenario (array packing issues gimme a tissue)
	{
		...
		l["coefficients"].Set<DRR::Float>( nCoef );
		DRR::Buffer buf{ std::move( l ) };
