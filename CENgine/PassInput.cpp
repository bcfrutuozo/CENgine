...
PassInput::PassInput(std::string registeredName_in)
	:
	registeredName(registeredName_in)
{
	...
	const bool nameCharsInvalid = std::all_of(registeredName.begin(), registeredName.end(), [](char c)
		{
			return std::isalnum(c) || c == '_';
		});

	if(!nameCharsInvalid || std::isdigit(registeredName.front()))
	{
		throw RGC_EXCEPTION("Invalid output name: " + registeredName);
	}
}
