...
SMBIOS::SMBIOS()
{
	...
	if(GetFirmwareType(&t) == 0)
	{
		throw std::runtime_error("Could not load the firmware type");
	}
	...
	switch(t)
	{
		...
		{
			if(buffer[0] == '_' && buffer[1] == 'S' && buffer[2] == 'M' && buffer[3] == '_')
			{
				if(buffer[5] != 0x1F)
				{
					throw std::runtime_error("Invalid SMBIOS 2.X entry point length");
				}
				if(buffer[10] != 0)
				{
					throw std::runtime_error("Invalid SMBIOS 2.X entry point revision");
				}
				if(buffer[16] != '_' || buffer[17] != 'D' || buffer[18] != 'M' || buffer[19] != 'I' || buffer[20] != '_')
				{
					throw std::runtime_error("Invalid SMBIOS 2.X intermediate anchor string. Was expecting \"_DMI_\"");
				}

				version_ = buffer[6] << 8 | buffer[7];
			}
			else if(buffer[0] == '_' && buffer[1] == 'S' && buffer[2] == 'M' && buffer[3] == '3' && buffer[4] == '_')
			{
				if(buffer[6] != 0x18)
				{
					throw std::runtime_error("Invalid SMBIOS 3.X entry point length");

					if(buffer[10] != 0x01)
					{
						throw std::runtime_error("Invalid SMBIOS 3.X entry point revision");
					}

					version_ = buffer[7] << 8 | buffer[8];
				}
			}
			break;
		}
		case FirmwareTypeUefi:
		{
			...
			version_ = smBiosData->SMBIOSMajorVersion << 8 | smBiosData->SMBIOSMinorVersion;
			data_ = smBiosData->SMBIOSTableData;
			size_ = smBiosData->Length;
		}
		break;
		default:
		throw std::runtime_error("Unsupported firmware type");
	}

	// is a valid version?
	if((version_ < SMBIOS_2_0 || version_ > SMBIOS_2_8) && version_ != SMBIOS_3_0)
	{
		throw std::runtime_error("Unsupported SMBIOS version");
	}
...
void SMBIOS::RaisePrivilege()
{
	...
	{
		throw std::runtime_error("Failed to raise privilege for the application");
	}
}
