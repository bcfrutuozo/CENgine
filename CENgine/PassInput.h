...
public:
	...
	void Bind(PassOutput& output) override
	{
		auto p = std::dynamic_pointer_cast<T>(output.YieldBuffer());
		
		if(!p)
		{
			std::ostringstream oss;
			oss << "Binding input [" << GetRegisteredName() << "] to output [" << GetPassName() << "." << GetOutputName() << "] "
				<< " { " << typeid(T).name() << " } not compatible with { " << typeid(*output.YieldBuffer().get()).name() << " }";
...
public:
	...
	void Bind(PassOutput& output) override
	{
		auto p = std::dynamic_pointer_cast<T>(output.YieldImmutable());

		if(!p)
		{
			std::ostringstream oss;
			oss << "Binding input [" << GetRegisteredName() << "] to output [" << GetPassName() << "." << GetOutputName() << "] "
				<< " { " << typeid(T).name() << " } does not match { " << typeid(*output.YieldImmutable().get()).name() << " }";
			throw RGC_EXCEPTION( oss.str() );
		}
