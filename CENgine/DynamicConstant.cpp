...
namespace DRR
{
	...
	void IncompleteLayout::ClearRoot() noexcept
	{
		*this = IncompleteLayout();
	}

	std::shared_ptr<LayoutElement> IncompleteLayout::DeliverRoot() noexcept
	{
	...
	CompleteLayout::CompleteLayout(std::shared_ptr<LayoutElement> pRoot) noexcept
		:
		Layout(std::move(pRoot))
	{ }

	std::shared_ptr<LayoutElement> CompleteLayout::RelinquishRoot() const noexcept
	{
		return std::move(pRoot);
	}

	std::shared_ptr<LayoutElement> CompleteLayout::ShareRoot() const noexcept
	{
		return pRoot;
	}

	const LayoutElement& CompleteLayout::operator[](const std::string& key) const NOXND
	{
		return (*pRoot)[key];
	}
	...
	ConstElementRef::Pointer ConstElementRef::operator&() const NOXND
	{
		return Pointer{ this };
	}
	...
	ConstElementRef::Pointer::Pointer(const ConstElementRef* ref) noexcept : ref(ref)
	{ }
	...
	ElementRef::Pointer ElementRef::operator&() const NOXND
	{
	...
	ElementRef::Pointer::Pointer(ElementRef* ref) noexcept : ref(ref)
	{ }

	Buffer::Buffer(IncompleteLayout&& lay) NOXND
		:
		Buffer(LayoutCodex::Resolve(std::move(lay)))
	{ }

	Buffer::Buffer(const CompleteLayout& lay) NOXND
		:
		pLayoutRoot(lay.ShareRoot()),
		bytes(pLayoutRoot->GetOffsetEnd())
	{ }

	Buffer::Buffer(CompleteLayout&& lay) NOXND
		:
